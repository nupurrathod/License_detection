...
def get_vehicle_info(plate_number):
    r = requests.get(
        "http://www.regcheck.org.uk/api/reg.asmx/CheckIndia?RegistrationNumber={0}&username=car21".format(str(plate_number)))
...
def plate_info(numberPlate):
    ...
    if len(numberPlate) > 10:
        ...
        x = get_vehicle_info(numberPlate)
        print(x)
        return x
#     else:
